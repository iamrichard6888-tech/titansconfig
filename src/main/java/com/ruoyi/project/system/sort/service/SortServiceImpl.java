package com.ruoyi.project.system.sort.service;
...
public class SortServiceImpl implements ISortService
{
    ...
    @Override
    @DataScope(deptAlias = "d")
    public List<ZtreeString> selectSortTree(Sort sort)
    {
        List<Sort> sortList = sortMapper.selectSortList(sort);
        List<ZtreeString> ztrees = initSortZtree(sortList);
        return ztrees;
    }
     ...
     * @return 树结构列表
     */
    public List<ZtreeString> initSortZtree(List<Sort> sortList)
    {
        return initSortZtree(sortList, null);
    }
     ...
     * @return 树结构列表
     */
    public List<ZtreeString> initSortZtree(List<Sort> sortList, List<String> roleDeptList)
    {
        ...
        for (Sort sort : sortList)
        {
            ...
            if(sort.getLevel()==2){
            	ztree.setName("("+sort.getCode()+")"+sort.getGroupName());
            }else{
            	ztree.setName(sort.getGroupName());
            }
            
            ztree.setTitle(sort.getGroupName());
            ztree.setType(sort.getTableName());
            ztree.setTypeid(sort.getTableId());
            ztrees.add(ztree);
        }
        return ztrees;
    }
    ...
    {
    	sortMapper.delAll();
    	List<Sort> sortList = sortMapper.selectUnitVo();
    	for(Sort sort:sortList){
    		String treeUnitid =IdUtils.simpleUUID();
    		
    		List<Sort> list1 = sortMapper.selectStrutstypeVo(sort);
    		for(Sort s1:list1){
    			String treeStrutstypeId = IdUtils.simpleUUID();
    			List<Sort> list2 = sortMapper.selectCarriertypeVo(s1);
    			for(Sort s2:list2){
    				String treeCarriertypeId = IdUtils.simpleUUID();
    				Sort ss = new Sort();
    				ss.setCode(s1.getCode());
    				ss.setTableName(s2.getCode());
    				ss.setTableId(sort.getId());
    				List<Sort> list3 = sortMapper.selectExtsortVo(ss);
    				for(Sort s3:list3){
    					s3.setParentId(treeCarriertypeId);
    					sortMapper.insertSort(s3);
    				}
    				
    				s2.setId(treeCarriertypeId);
        			s2.setParentId(treeStrutstypeId);
        			sortMapper.insertSort(s2);
    			}
    			s1.setId(treeStrutstypeId);
    			s1.setParentId(treeUnitid);
    			sortMapper.insertSort(s1);
    		}
    		sort.setGroupName(sort.getCode()+sort.getGroupName());
    		sort.setId(treeUnitid);
