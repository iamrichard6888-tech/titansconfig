...
public class PostController extends BaseController
{
    ...
    @PostMapping("/add")
    @ResponseBody//这个是和上面的区别，这个是有响应体的，所以这个会返回ajax的结果。上面的add只是单纯的返回一个路径，这个路径是一个页面资源
    public AjaxResult addSave(@Validated Post post)//这个validate是和实体类中的成员变量的校验注解一起用的，他validate会按照校验注解校验
    {
